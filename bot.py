...
ADMIN_ID = @kate_lic  # ✅ ← сюда вставь свой Telegram ID (из @userinfobot)
...
# ====== ✅ ДОБАВЛЕНО: автопроверка вебхука ======
async def ensure_webhook():
    info = await bot.get_webhook_info()
    if not info.url:
        await bot.set_webhook(WEBHOOK_URL)
        logging.info(f"Webhook восстановлен: {WEBHOOK_URL}")
        try:
            await bot.send_message(ADMIN_ID, f"✅ Вебхук восстановлен:\n{WEBHOOK_URL}")
...
async def on_shutdown(app):
    ...
    try:
        await bot.send_message(ADMIN_ID, "🛑 Бот остановлен или сервер перезагружается.")
    except Exception as e:
        logging.error(f"Не удалось уведомить администратора об остановке: {e}")
